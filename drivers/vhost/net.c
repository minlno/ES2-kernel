			    ...
			    unsigned long arg)
{
	...
	case VHOST_NET_SET_BACKEND:
		if (DEBUG_MH)
			printk(KERN_INFO "%s: VHOST_NET_SET_BACKEND\n", __func__);
		if (copy_from_user(&backend, argp, sizeof backend))
			return -EFAULT;
		return vhost_net_set_backend(n, backend.index, backend.fd);
	case VHOST_GET_FEATURES:
		if (DEBUG_MH)
			printk(KERN_INFO "%s: VHOST_GET_FEATURES\n", __func__);
	...
	case VHOST_SET_FEATURES:
		if (DEBUG_MH)
			printk(KERN_INFO "%s: VHOST_SET_FEATURES\n", __func__);
	...
	case VHOST_GET_BACKEND_FEATURES:
		if (DEBUG_MH)
			printk(KERN_INFO "%s: VHOST_GET_BACKEND_FEATURES\n", __func__);
	...
	case VHOST_SET_BACKEND_FEATURES:
		if (DEBUG_MH)
			printk(KERN_INFO "%s: VHOST_SET_BACKEND_FEATURES\n", __func__);
		if (copy_from_user(&features, featurep, sizeof(features)))
			return -EFAULT;
	...
	case VHOST_RESET_OWNER:
		if (DEBUG_MH)
			printk(KERN_INFO "%s: VHOST_RESET_OWNER\n", __func__);
		return vhost_net_reset_owner(n);
	case VHOST_SET_OWNER:
		if (DEBUG_MH)
			printk(KERN_INFO "%s: VHOST_SET_OWNER\n", __func__);
	...
	default:
		...
		if (r == -ENOIOCTLCMD) {
			if (DEBUG_MH)
				printk(KERN_INFO "%s: default, vhost_vring_ioctl\n", __func__);
			r = vhost_vring_ioctl(&n->dev, ioctl, argp);
		} else {
			if (DEBUG_MH)
				printk(KERN_INFO "%s: default, vhost_net_flush\n", __func__);
			vhost_net_flush(n);
		}
		mutex_unlock(&n->dev.mutex);
		return r;
